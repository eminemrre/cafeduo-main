...
- Latest validation: `npm audit --audit-level=moderate`, workflow/OpenAPI/compose YAML parse, backend/migration syntax checks, `test:ci` (812 tests), CI-equivalent `test:coverage` (812 tests), `build`, and `test:e2e` (14 tests) passed; deploy workflow YAML re-checked after port/public-smoke fallback.
