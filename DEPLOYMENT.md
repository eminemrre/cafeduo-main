...
| `***REMOVED***` | PostgreSQL connection string | `postgres://user:pass@localhost:5432/cafeduo` |
| `***REMOVED***` | Secret key for JWT tokens | Generate with: `node -e "console.log(require('crypto').randomBytes(64).toString('hex'))"` |
...
***REMOVED***=postgres://username:password@hostname:5432/database_name
...
```bash
# JWT Settings
***REMOVED***=your-super-secret-jwt-key-change-this-in-production
JWT_EXPIRES_IN=7d
...
export PORT=3001
export ***REMOVED***=postgres://user:pass@localhost:5432/cafeduo
export ***REMOVED***=your-secret-key
...
# Manual backup
pg_dump $***REMOVED*** > cafeduo_backup.sql

# Manual restore
psql $***REMOVED*** < cafeduo_backup.sql
```
...
psql $***REMOVED*** -c "CREATE INDEX IF NOT EXISTS idx_users_email ON users(email);"
psql $***REMOVED*** -c "CREATE INDEX IF NOT EXISTS idx_games_status ON games(status);"
psql $***REMOVED*** -c "CREATE INDEX IF NOT EXISTS idx_user_items_user_id ON user_items(user_id);"
```
...
3. Test database connection: `psql $***REMOVED*** -c "SELECT 1;"`
...
- [ ] Changed default ***REMOVED*** to a strong random string
