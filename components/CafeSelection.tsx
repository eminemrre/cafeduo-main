...
export const CafeSelection: React.FC<CafeSelectionProps> = ({ currentUser, onCheckInSuccess }) => {
  ...
  return (
    <div className="min-h-screen rf-page-shell text-[var(--rf-ink)] pt-24 pb-[calc(2.5rem+env(safe-area-inset-bottom))] px-4 font-sans relative overflow-hidden noise-bg">
      <div className="absolute inset-0 rf-grid opacity-[0.06] pointer-events-none" />
      <div className="max-w-md w-full mx-auto rf-screen-card p-6 sm:p-8 relative overflow-hidden rf-elevated">
        ...
        <div className="space-y-5" aria-busy={loading} aria-live="polite">
          ...
          <button
            ...
            className={`w-full py-4 font-bold text-white flex items-center justify-center gap-2 transition-all rf-control border-2 ${
              loading || !tableNumber
