...
export const GladiatorGame: React.FC<GladiatorGameProps> = ({ currentUser, opponentName, isBot, onGameEnd, onLeave }) => {
    ...
    // Timer Logic
    useEffect(() => {
        if (turn === 'player' && !gameOver && !animating) {
            setTimeLeft(15);
    ...
    const executeMove = (attacker: 'player' | 'opponent', action: ActionType) => {
        setAnimating(true);
